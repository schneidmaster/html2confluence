...
describe HTMLToConfluenceParser, "when converting html to textile" do
  ...
  before :all do
    ...
    #puts @confluence
    #puts RedCloth.new(@confluence).to_html
  end
  ...
  it "should convert paragraph tags" do
    # We don't include paragraph classes @confluence.should match(/^p(\([^\)]+\))?\./)
  end
  ...
  it "should convert class and id attributes" do
    # We don't convert classes. @confluence.should include("h1(story.title entry-title#post-312).")
  end
  ...
  xit "should escape special characters" do
